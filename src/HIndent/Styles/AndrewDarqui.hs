...
-- | Andrew Darqui's Style.
-- Based heavily (code rip) off of Cramer & Chris Done's style, with slight modifications.

module HIndent.Styles.AndrewDarqui (adarqui) where
...
-- | Printer state.
data State =
  State {adarquiLineBreak :: LineBreak     -- ^ Current line breaking mode
        ,adarquiLangPragmaLength :: Int    -- ^ Padding length for pragmas
        ,adarquiModuleImportLength :: Int  -- ^ Padding length for module imports
        ,adarquiRecordFieldLength :: Int   -- ^ Padding length for record fields
        }
  deriving (Show)
...
adarquiDefaultConfig :: Config
adarquiDefaultConfig = defaultConfig {configMaxColumns = 120
...
adarqui :: Style
adarqui =
  Style {styleName = "adarqui"
        ,styleAuthor = "Andrew Darqui"
        ,styleDescription = "Andrew Darqui's style"
        ,styleInitialState =
           State {adarquiLineBreak = Free
                 ,adarquiLangPragmaLength = 0
                 ,adarquiModuleImportLength = 0
                 ,adarquiRecordFieldLength = 0}
        ,styleExtenders =
           [Extender extModule
        ...
        ,styleDefConfig = adarquiDefaultConfig
...
-- | Set indentation level to specific column
alignCol :: MonadState (PrintState s) m
      => Int64 -> m a -> m a
alignCol col p =
  do st <- get
     column col p
...
withLineBreak lb p =
  do old <- gets (adarquiLineBreak . psUserState)
     modifyState $ \s -> s {adarquiLineBreak = lb}
     result <- p
     modifyState $ \s -> s {adarquiLineBreak = old}
     return result
...
attemptSingleLine single multi =
  do prevState <- get
     case adarquiLineBreak . psUserState $ prevState of
       Single -> single
       Multi -> multi
       Free ->
...
reduceIndent short long printer =
  do linebreak <- gets (adarquiLineBreak . psUserState)
     case linebreak of
       Single -> single
       Multi -> multi
       Free ->
...
typeSig ty =
  attemptSingleLineType
    (write " :: " >> pretty ty)
    (newline >> space2 >> (align' $ write ":: " >> pretty ty))

--    (newline >> (align' $ write ":: " >> pretty ty))
--                        (newline >> (alignCol (configIndentSpaces adarquiDefaultConfig) $ write ":: " >> pretty ty))
--                        (newline >> alignCol (configIndentSpaces adarquiDefaultConfig) $ write ":: " >> pretty ty)
...
extModule (Module _ mhead pragmas imports decls) =
  do modifyState $ \s -> s {adarquiLangPragmaLength = pragLen
                           ,adarquiModuleImportLength = modLen}
     inter (newline >> newline) $
       catMaybes [unless' (null pragmas) $ preserveLineSpacing pragmas
                 ,pretty <$> mhead
                 ,unless' (null imports) $ nl2 *> preserveLineSpacing imports
                 ,unless' (null decls) $
                  do forM_ (init decls) $
                       \decl ->
                         do pretty decl
...
extModulePragma (LanguagePragma _ names) =
  do namelen <- gets (adarquiLangPragmaLength . psUserState)
     forM_ names $
       \name ->
         do write "{-# LANGUAGE "
...
extExportSpecList (ExportSpecList _ exports) =
  case exports of
    [] -> write " ()"
    [e]
      | not (hasComments (const True) e) -> do
          write " ("
          newline
          write "  " >> pretty e
          newline
...
extImportDecl ImportDecl{..} =
  do if importQualified
     ...
     namelen <- gets (adarquiModuleImportLength . psUserState)
...
extDecl (FunBind  _ matches) = preserveLineSpacing matches -- TODO *> nl2
...
extDecl (PatBind _ pat rhs mbinds) =
  do pretty pat
     ...
     -- TODO nl2
extDecl other = nl2col0 *> prettyNoExt other -- TODO *> nl2
...
extConDecl (RecDecl _ name fields) =
  do modifyState $ \s -> s {adarquiRecordFieldLength = fieldLen}
     pretty name
     space
     case fields of
       [] -> write "{ }"
...
extFieldDecl (FieldDecl _ [name] ty) =
  do namelen <- gets (adarquiRecordFieldLength . psUserState)
     string $ padRight namelen $ nameStr name
     space
     typeSig ty
...
nl2, nl3 :: Printer State ()
nl2 = newline *> newline
nl3 = nl2 *> newline



space2 :: Printer State ()
space2 = space *> space

nl2col0 :: Printer State ()
nl2col0 =
  do st <- get
     let col =
           if psEolComment st
